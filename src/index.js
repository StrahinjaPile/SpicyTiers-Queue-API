...
// Increased so players have enough time to connect
// from the matchmaking screen to the duel server.
...
async function createMatch(
  ...
  /*
   * Everything is put into one D1 batch.
   *
   * IMPORTANT:
   * We also check that both players are still queued.
   * This prevents a stale polling request from creating
   * a match after somebody already left.
   */

  const statements = [

    // Delete player 1 only if still queued
    env.DB.prepare(`
      DELETE FROM queue
      WHERE uuid = ?
        AND mode = ?
    ...
    // Delete player 2 only if still queued
    env.DB.prepare(`
      DELETE FROM queue
      WHERE uuid = ?
        AND mode = ?
...
//
// /match/status now calls this function.
//
// That means:
// 1. Player is already in queue.
// 2. Another player is in queue.
// 3. Server was unavailable.
// 4. Server becomes available.
// 5. Client polls /match/status.
// 6. This function finds opponent + server.
// 7. Match is created.
// 8. Client receives server address.
// 9. Fabric automatically connects.
//

async function tryMatchQueuedPlayer(
  env,
  uuid
) {
  console.log(
  ...
  // If player region is auto and no server was found
  // with the player's region, try any available server.
  if (
    !server &&
    player.region !== "auto"
  ) {
    console.log(
      "[RETRY] No server in selected region."
    );
  ...
  if (!server) {
    console.log(
      "[RETRY] Opponent exists but no server is available."
    );
  ...
  const playerExistingMatch =
    await getMatchByPlayer(
      env,
      player.uuid
    );

  if (playerExistingMatch) {
    console.log(
      "[RETRY] Player already has match:",
      playerExistingMatch.id
    );
  ...
  const opponentExistingMatch =
    await getMatchByPlayer(
      env,
      opponent.uuid
    );

  if (opponentExistingMatch) {
    console.log(
      "[RETRY] Opponent already has match:",
      opponentExistingMatch.id
    );
  ...
  // -------------------------
  // Verify both are STILL queued
  // -------------------------

  const playerStillQueued =
    await env.DB.prepare(`
      SELECT id
      FROM queue
      WHERE uuid = ?
  ...
  if (
    ...
    console.log(
      "[RETRY] One of the players is no longer queued."
    );
  ...
  try {
    ...
    if (!match) {
      throw new Error(
        "Match was created but could not be loaded: " +
        matchId
      );
    }
...
export default {
  async fetch(
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          if (
            ...
            /*
             * IMPORTANT:
             * Don't simply return here.
             *
             * Try matchmaking again immediately.
             */
            const retryResult =
              await tryMatchQueuedPlayer(
                env,
                uuid
              );
          ...
          if (
            ...
            /*
             * We do NOT require a server here.
             *
             * The player can enter queue even if
             * the duel server is temporarily unavailable.
             *
             * This is important because /match/status
             * will retry server allocation later.
             */
      ...
      //
      // Every poll now retries matchmaking.
      //
